...
class Render:
    def __init__(self):
        ...
        if current_time[0] == '0' and (current_time[1] >= '1' and current_time[1] <= '7'):
            sudo /home/pi/My-NBA-scoreboard/rpi-rgb-led-matrix/examples-api-use/demo -D 7 --led-rows=32 --led-cols=64 --led-parallel=2 --led-pwm-bits=9 --led-slowdown-gpio=2 --led-chain=2 --led-no-hardware-pulse
                ...
                if timeremaining[0] == 'Q' and (timeremaining[1] >= '4' and (timeremaining[3] == '0' and timeremaining[4] <= '4')): #Q4 or OT < 5min remaining
                    if homescore > awayscore:
                        if (homescore - awayscore) <= 15: #close game
                            graphics.DrawText(canvas, self.font3, 2, 54, graphics.Color(255, 255, 255), game['gameStatusText']) #bright quarter and time remaining
