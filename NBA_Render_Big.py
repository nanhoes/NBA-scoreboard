...
class Render:
    def __init__(self):
        ...
        self.font2.LoadFont("/home/pi/rpi-rgb-led-matrix/fonts/8x13.bdf")
    ...
    def Render_Games(self, printer=False):
        ...
        for game in game_data:
            ...
            for line in range(0,18):
                graphics.DrawLine(canvas, 0, line, 36, line, graphics.Color(self.team_colors[awayteam][0][0], self.team_colors[awayteam][0][1], self.team_colors[awayteam][0][2]))
            graphics.DrawText(canvas, self.font2, 128 - len(str(over_under))*4, 14, graphics.Color(0, 0, 200), over_under)
            graphics.DrawText(canvas, self.font2, 128 - len(str(spread))*4, 34, graphics.Color(0, 0, 200), spread)
            graphics.DrawText(canvas, self.font, 1, 36, graphics.Color(self.team_colors[hometeam][1][0], self.team_colors[hometeam][1][1], self.team_colors[hometeam][1][2]), hometeam)
            graphics.DrawText(canvas, self.font, 1, 16, graphics.Color(self.team_colors[awayteam][1][0], self.team_colors[awayteam][1][1], self.team_colors[awayteam][1][2]), awayteam)
            ...
            if game['gameStatus'] == 3: #finished game
                graphics.DrawText(canvas, self.font, 1, 56, graphics.Color(255, 0, 0), game['gameStatusText']) #red '"final" text
                if homescore > awayscore:
            ...
            if game['gameStatus'] == 1: #upcoming game
                ...
                graphics.DrawText(canvas, self.font2, 42, 14, graphics.Color(100, 100, 100), awayrecord) #away team record
                graphics.DrawText(canvas, self.font2, 62, 34, graphics.Color(100, 100, 100), homerecord) #home team record
                if game['gameStatusText'] != 'PPD': #upcoming game
                    graphics.DrawText(canvas, self.font, 1, 56, graphics.Color(100, 100, 100), game['gameStatusText'][0:len(game['gameStatusText']) - 3])
                if game['gameStatusText'] == 'PPD': #postponed game
                    graphics.DrawText(canvas, self.font, 1, 56, graphics.Color(100, 100, 100), 'Postponed')
