...
sudo ${install_path}/rpi-rgb-led-matrix/utils/led-image-viewer --led-gpio-mapping=${hardware_mapping} --led-cols=${columns} --led-chain=${chain_length} --led-parallel=${parallel} --led-gpio-mapping=${hardware_mapping} --led-slowdown-gpio=${gpio_slowdown} --led-brightness=${brightness} --led-row-addr-type=${row_address_type} ${install_path}/images/${gif}.gif
